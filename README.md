...
メインの応答終了時は「完了したのだ」、質問画面を開くときは「追加の質問があるのだ」、中断時（手動中断を含む）は「処理が中断されたのだ」を再生します。子エージェントは無音です。音声クレジットは **VOICEVOX:ずんだもん** です。[音声テンプレートと再生成手順](hooks/sounds/zundamon/README.md)を参照してください。
