...
### 1-6. 声のある下書き — 新規執筆・大幅改稿

新規執筆、または構成から組み直す大幅改稿では、`references/voice-first-draft.md` を読み、話者の立場・具体性・自然な濃淡を備えた内部下書きを先に作る。その下書きを§2以降の入力にする。短い校正、局所的な修正、診断だけを行う score モードでは、この前段を省略する。
