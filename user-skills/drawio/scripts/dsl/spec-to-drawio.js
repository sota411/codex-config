...
function normalizeSemanticType(type) {
  ...
  return TYPE_ALIASES[type] || TYPE_ALIASES[type.toLowerCase()] || type
}
...
export function detectSemanticType(label, explicitType, network = null) {
  const normalizedExplicitType = normalizeSemanticType(explicitType)
  if (normalizedExplicitType && SHAPE_STYLES[normalizedExplicitType]) {
    return normalizedExplicitType
  }

  const normalizedDevice = normalizeSemanticType(network?.device)
  if (normalizedDevice && SHAPE_STYLES[normalizedDevice]) {
...
const SIZE_PRESETS = {
  ...
  uml_class: { width: 180, height: 120 },
...
function isUmlClassNode(node) {
  return detectSemanticType(node.label, node.type, node.network) === 'umlClass'
}

function buildUmlClassLabel(node) {
  const stereotypes = normalizeTextList(node.stereotypes || node.stereotype)
  const attributes = normalizeTextList(node.attributes || node.uml?.attributes)
  const operations = normalizeTextList(node.operations || node.methods || node.uml?.operations || node.uml?.methods)

  const titleLines = [
    ...stereotypes.map(stereotype => `&lt;&lt;${escapeHtmlText(stereotype)}&gt;&gt;`),
    `<b>${escapeHtmlText(node.label)}</b>`
  ]

  const sections = [
    `<div style="text-align:center">${titleLines.join('<br/>')}</div>`
  ]

  if (attributes.length > 0 || operations.length > 0) {
    sections.push(attributes.map(escapeHtmlText).join('<br/>'))
  }
  if (operations.length > 0) {
    sections.push(operations.map(escapeHtmlText).join('<br/>'))
  }

  return escapeXml(sections.join('<hr size="1"/>'))
}

function formatNodeValue(node) {
  if (isUmlClassNode(node)) {
    return buildUmlClassLabel(node)
  }
  return prepareMathLabel(node.label)
}
...
export function generateConnectorStyle(edge, theme, routing = 'orthogonal') {
  ...
  const fontColor = resolveThemeColor(edge.style?.fontColor, theme, connectorTheme.fontColor || theme.colors?.textMuted || '#64748B')
  const fontSize = edge.style?.fontSize || connectorTheme.fontSize || 11
...
export function buildXml(spec, theme, layout) {
  ...
  const expandCanvasPoint = (point, padding = 0) => {
    if (!point) return
    if (Number.isFinite(point.x)) minX = Math.min(minX, point.x - padding)
    if (Number.isFinite(point.y)) minY = Math.min(minY, point.y - padding)
    if (Number.isFinite(point.x)) maxX = Math.max(maxX, point.x + padding)
    if (Number.isFinite(point.y)) maxY = Math.max(maxY, point.y + padding)
  }
  ...
  const pointOnPath = (points, ratio) => {
    ...
    const target = total * Math.min(Math.max(ratio, 0), 1)
    let walked = 0
  ...
  const edgeConnectionPoint = (pos, style, xKey, yKey, dxKey, dyKey) => {
    const xRatio = Number(style?.[xKey])
    const yRatio = Number(style?.[yKey])
    if (!Number.isFinite(xRatio) || !Number.isFinite(yRatio)) {
      return centerOf(pos)
    }

    const dx = Number(style?.[dxKey])
    const dy = Number(style?.[dyKey])
    return {
      x: pos.x + pos.width * xRatio + (Number.isFinite(dx) ? dx : 0),
      y: pos.y + pos.height * yRatio + (Number.isFinite(dy) ? dy : 0)
    }
  }

  const edgePathPoints = (edge, sourcePos, targetPos) => {
    if (edge.waypoints?.length) {
      return [centerOf(sourcePos), ...edge.waypoints, centerOf(targetPos)]
    }

    return [
      edgeConnectionPoint(sourcePos, edge.style, 'exitX', 'exitY', 'exitDx', 'exitDy'),
      edgeConnectionPoint(targetPos, edge.style, 'entryX', 'entryY', 'entryDx', 'entryDy')
    ]
  }

  const edgeLabelRatio = (edge) => edge.labelPosition === 'start' ? 0.2
    : edge.labelPosition === 'end' ? 0.8
      : 0.5

  const computeEdgeLabelOffset = (edge, sourcePos, targetPos) => {
    const labelBase = pointOnPath(edgePathPoints(edge, sourcePos, targetPos), edgeLabelRatio(edge))
    if (!labelBase) return { x: 0, y: -12 }

    if (edge.__routing?.orientation === 'vertical') {
      const overlappingColumns = [sourcePos, targetPos].filter(pos =>
        labelBase.x >= pos.x && labelBase.x <= pos.x + pos.width
      )
      if (overlappingColumns.length === 0) {
        return { x: 12, y: 0 }
      }

      const rightEdge = Math.max(...overlappingColumns.map(pos => pos.x + pos.width))
      return {
        x: Math.max(12, rightEdge - labelBase.x + 12),
        y: 0
      }
    }

    const overlappingBands = [sourcePos, targetPos].filter(pos =>
      labelBase.y >= pos.y && labelBase.y <= pos.y + pos.height
    )
    if (overlappingBands.length === 0) {
      return { x: 0, y: -12 }
    }

    const topEdge = Math.min(...overlappingBands.map(pos => pos.y))
    return {
      x: 0,
      y: Math.min(-12, topEdge - labelBase.y - 8)
    }
  }
  ...
  for (const edge of routedEdges) {
    ...
    const edgePath = edgePathPoints(edge, sourcePos, targetPos)

    for (const point of edgePath) {
      expandCanvasPoint(point)
    }

    const rawEdgeLabel = formatNetworkEdgeLabel(edge)
    if (rawEdgeLabel) {
      const labelPoint = pointOnPath(edgePath, edgeLabelRatio(edge))
      const labelOffset = computeEdgeLabelOffset(edge, sourcePos, targetPos)
      expandCanvasPoint({
        x: labelPoint.x + labelOffset.x,
        y: labelPoint.y + labelOffset.y
      }, rawEdgeLabel.length * 4)
    }
  }

  const translateX = minX < 0 ? Math.ceil((-minX + 40) / 8) * 8 : 0
  const translateY = minY < 0 ? Math.ceil((-minY + 40) / 8) * 8 : 0
  const canvasWidth = snapToGrid(maxX + translateX + 80, 8)
  const canvasHeight = snapToGrid(maxY + translateY + 80, 8)
  ...
  for (const [moduleId, pos] of modulePositions) {
    ...
    cells.push(
      ...
      `<mxGeometry x="${pos.x + translateX}" y="${pos.y + translateY}" width="${pos.width}" height="${pos.height}" as="geometry"/>` +
      `</mxCell>`
    )
  }
  ...
  for (const node of spec.nodes || []) {
    ...
    const label = formatNodeValue(node)
    ...
    let x = pos.x + translateX
    let y = pos.y + translateY
    if (parentId !== '1') {
  ...
  for (const edge of routedEdges) {
    ...
    const sourcePos = positions.get(edge.from)
    const targetPos = positions.get(edge.to)
    const labelX = String(edgeLabelRatio(edge))
    const labelOffsetPoint = sourcePos && targetPos
      ? computeEdgeLabelOffset(edge, sourcePos, targetPos)
      : { x: 0, y: -12 }
    const labelOffset = `<mxPoint x="${labelOffsetPoint.x}" y="${labelOffsetPoint.y}" as="offset"/>`

    let edgeXml = `<mxCell id="${cellId}" value="${edgeLabel}" style="${style}" edge="1" parent="1" source="${sourceId}" target="${targetId}">`
    edgeXml += rawEdgeLabel
      ? `<mxGeometry x="${labelX}" relative="1" as="geometry">`
      : '<mxGeometry relative="1" as="geometry">'
    if (edge.waypoints?.length) {
      edgeXml += '<Array as="points">'
      for (const point of edge.waypoints) {
        edgeXml += `<mxPoint x="${point.x + translateX}" y="${point.y + translateY}"/>`
      }
      edgeXml += '</Array>'
    }
    if (rawEdgeLabel) {
...
export function validateSpec(spec) {
  ...
  for (const node of spec.nodes) {
    ...
    if (node.type != null && !SHAPE_STYLES[normalizeSemanticType(node.type)]) {
