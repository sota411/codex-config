...
describe('detectSemanticType', () => {
  ...
  it('should normalize UML class type aliases', () => {
    assert.strictEqual(detectSemanticType('Customer', 'umlClass'), 'umlClass')
    assert.strictEqual(detectSemanticType('Customer', 'uml-class'), 'umlClass')
    assert.strictEqual(detectSemanticType('Customer', 'uml_class'), 'umlClass')
    assert.strictEqual(detectSemanticType('Customer', 'class'), 'umlClass')
  })
})
...
describe('getNodeSize', () => {
  ...
  it('should default UML class nodes to a compartment-friendly size', () => {
    assert.deepStrictEqual(getNodeSize(null, 'umlClass'), { width: 180, height: 120 })
  })
})
...
describe('generateNodeStyle', () => {
  ...
  it('should generate classic UML class style', () => {
    const classicTheme = loadTheme('uml-classic')
    const style = generateNodeStyle({ id: 'customer', label: 'Customer', type: 'umlClass' }, classicTheme)
    assert.ok(style.includes('umlClass=1'), 'should mark the node as UML class for SVG rendering')
    assert.ok(style.includes('rounded=0'), 'should use square corners')
    assert.ok(style.includes('fillColor=#FFFFFF'), 'should use white fill')
    assert.ok(style.includes('strokeColor=#000000'), 'should use black stroke')
    assert.ok(style.includes('verticalAlign=top'), 'should align compartment text to the top')
    assert.ok(style.includes('align=left'), 'should align class members to the left')
  })
})
...
describe('generateConnectorStyle', () => {
  ...
  it('should generate UML relationship styles from the classic UML theme', () => {
    const classicTheme = loadTheme('uml-classic')

    const association = generateConnectorStyle({ from: 'a', to: 'b', type: 'association' }, classicTheme)
    assert.ok(association.includes('endArrow=none'), 'association should have no arrowhead')
    assert.ok(!association.includes('dashed=1'), 'association should be solid')

    const composition = generateConnectorStyle({ from: 'whole', to: 'part', type: 'composition' }, classicTheme)
    assert.ok(composition.includes('startArrow=diamond'), 'composition should place a diamond at the source')
    assert.ok(composition.includes('startFill=1'), 'composition diamond should be filled')

    const aggregation = generateConnectorStyle({ from: 'whole', to: 'part', type: 'aggregation' }, classicTheme)
    assert.ok(aggregation.includes('startArrow=diamond'), 'aggregation should place a diamond at the source')
    assert.ok(aggregation.includes('startFill=0'), 'aggregation diamond should be hollow')

    const realization = generateConnectorStyle({ from: 'impl', to: 'iface', type: 'realization' }, classicTheme)
    assert.ok(realization.includes('dashed=1'), 'realization should be dashed')
    assert.ok(realization.includes('endArrow=block'), 'realization should use a triangle-like arrow')
    assert.ok(realization.includes('endFill=0'), 'realization arrow should be hollow')
  })
})
...
describe('specToDrawioXml', () => {
  ...
  it('should generate compartmented classic UML class nodes', () => {
    const spec = {
      meta: { theme: 'uml-classic', layout: 'horizontal' },
      nodes: [
        {
          id: 'customer',
          label: 'Customer',
      ...
      edges: [
        { from: 'customer', to: 'order', type: 'association', label: 'places' }
      ]
    }

    const xml = specToDrawioXml(spec)
    assert.ok(xml.includes('umlClass=1'), 'class nodes should be marked for UML rendering')
    assert.ok(xml.includes('&lt;b&gt;Customer&lt;/b&gt;'), 'class name should be formatted as title compartment')
    assert.ok(xml.includes('&lt;hr size=&quot;1&quot;/&gt;'), 'class label should include compartment separators')
    assert.ok(xml.includes('+ id: String'), 'attributes should be included')
    assert.ok(xml.includes('+ placeOrder(): Order'), 'operations should be included')
    assert.ok(xml.includes('endArrow=none'), 'association should have no arrowhead')
  })

  it('should accept UML class aliases through spec validation', () => {
    const spec = {
      meta: { theme: 'uml-classic' },
      nodes: [
        { id: 'a', label: 'AliasA', type: 'class' },
        { id: 'b', label: 'AliasB', type: 'uml-class' },
        { id: 'c', label: 'AliasC', type: 'uml_class' }
      ],
      edges: [
        { from: 'a', to: 'b', type: 'association' },
        { from: 'b', to: 'c', type: 'association' }
      ]
    }

    const xml = specToDrawioXml(spec)
    const umlClassCount = (xml.match(/umlClass=1/g) || []).length
    assert.strictEqual(umlClassCount, 3, 'all UML class aliases should render as umlClass nodes')
  })
...
`
  ...
  it('should parse and validate UML class type aliases', () => {
    const yamlText = `
meta:
  theme: uml-classic
nodes:
...
`
    ...
    assert.deepStrictEqual(
      spec.nodes.map(node => node.type),
      ['class', 'uml-class', 'uml_class'],
      'parseSpecYaml should accept UML class aliases without validation errors'
    )
  })
...
describe('loadTheme', () => {
  ...
  it('loadTheme("uml-classic") returns the classic UML theme', () => {
    const theme = loadTheme('uml-classic')
    assert.strictEqual(theme.name, 'uml-classic')
    assert.strictEqual(theme.node.umlClass.fillColor, '#FFFFFF')
    assert.strictEqual(theme.connector.composition.startArrow, 'diamond')
  })
...
describe('Phase 2.3: edge labelPosition', () => {
  ...
  it('horizontal edge labels should be offset above the connected node band', () => {
    const spec = {
      meta: { theme: 'tech-blue' },
      nodes: [
        { id: 'n1', label: 'A', position: { x: 120, y: 120 } },
        { id: 'n2', label: 'B', position: { x: 280, y: 120 } }
      ],
      edges: [{ from: 'n1', to: 'n2', label: 'label' }]
    }
    const xml = specToDrawioXml(spec)
    const offsetY = Number(xml.match(/<mxPoint x="0" y="(-?\d+(?:\.\d+)?)" as="offset"\/>/)[1])
    assert.ok(offsetY <= -20, `horizontal label offset should clear node band, got ${offsetY}`)
  })

  it('vertical edge labels should be offset outside the connected node column', () => {
    const spec = {
      meta: { theme: 'tech-blue' },
      nodes: [
        { id: 'n1', label: 'A', position: { x: 120, y: 120 } },
        { id: 'n2', label: 'B', position: { x: 120, y: 280 } }
      ],
      edges: [{ from: 'n1', to: 'n2', label: 'label' }]
    }
    const xml = specToDrawioXml(spec)
    const offsetX = Number(xml.match(/<mxPoint x="(\d+(?:\.\d+)?)" y="0" as="offset"\/>/)[1])
    assert.ok(offsetX >= 60, `vertical label offset should clear node column, got ${offsetX}`)
  })

  it('waypoint horizontal edge labels should clear the connected node band', () => {
    const spec = {
      meta: { theme: 'tech-blue' },
      nodes: [
        { id: 'n1', label: 'A', position: { x: 120, y: 120 } },
        { id: 'n2', label: 'B', position: { x: 360, y: 120 } }
      ],
      edges: [{
        from: 'n1',
        to: 'n2',
        label: 'flow',
        waypoints: [{ x: 240, y: 120 }]
      }]
    }
    const xml = specToDrawioXml(spec)
    const offsetY = Number(xml.match(/<mxPoint x="0" y="(-?\d+(?:\.\d+)?)" as="offset"\/>/)[1])
    assert.ok(offsetY <= -20, `waypoint horizontal label offset should clear node band, got ${offsetY}`)
  })

  it('waypoint vertical edge labels should clear the connected node column', () => {
    const spec = {
      meta: { theme: 'tech-blue', layout: 'vertical' },
      nodes: [
        { id: 'n1', label: 'A', position: { x: 120, y: 120 } },
        { id: 'n2', label: 'B', position: { x: 120, y: 360 } }
      ],
      edges: [{
        from: 'n1',
        to: 'n2',
    ...
    const xml = specToDrawioXml(spec)
    const offsetX = Number(xml.match(/<mxPoint x="(\d+(?:\.\d+)?)" y="0" as="offset"\/>/)[1])
    assert.ok(offsetX >= 60, `waypoint vertical label offset should clear node column, got ${offsetX}`)
  })
})
...
describe('edge label in XML', () => {
  ...
  it('edge with label keeps the label on the edge cell instead of adding a child edgeLabel cell', () => {
    const spec = {
      meta: { theme: 'tech-blue' },
      nodes: [
        { id: 'n1', label: 'A' },
    ...
    const xml = specToDrawioXml(spec)
    assert.ok(xml.includes('value="MyEdgeLabel"'), 'edge cell should carry the label text')
    assert.ok(!xml.includes('edgeLabel'), 'XML should not contain a duplicate edgeLabel child cell')
  })
  ...
  it('canvas size includes edge waypoints outside node bounds', () => {
    const spec = {
      meta: { theme: 'tech-blue' },
      nodes: [
        { id: 'n1', label: 'A', position: { x: 80, y: 80 } },
        { id: 'n2', label: 'B', position: { x: 240, y: 80 } }
      ],
      edges: [
        {
          from: 'n1',
          to: 'n2',
          waypoints: [{ x: 1000, y: 1000 }]
        }
      ]
    }
    const xml = specToDrawioXml(spec)
    const width = Number(xml.match(/pageWidth="(\d+)"/)[1])
    const height = Number(xml.match(/pageHeight="(\d+)"/)[1])
    assert.ok(width >= 1080, `pageWidth should include waypoint bounds, got ${width}`)
    assert.ok(height >= 1080, `pageHeight should include waypoint bounds, got ${height}`)
  })

  it('negative edge waypoints are translated into the draw.io page bounds', () => {
    const spec = {
      meta: { theme: 'tech-blue' },
      nodes: [
        { id: 'n1', label: 'A', position: { x: 80, y: 80 } },
    ...
    const xml = specToDrawioXml(spec)
    assert.ok(!xml.includes('x="-200"'), 'translated XML should not keep a negative waypoint x coordinate')
    assert.ok(!xml.includes('y="-100"'), 'translated XML should not keep a negative waypoint y coordinate')
    assert.ok(xml.includes('<mxPoint x="40" y="44"/>'), 'negative waypoint should be translated inside the page with margin')
  })
})
