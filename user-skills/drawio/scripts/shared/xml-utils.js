...
/**
 * Parse an edge label offset point from an <mxGeometry> body.
 * @param {string} geometryBody - inner HTML of <mxGeometry>
 * @returns {{x: number, y: number}}
 */
export function parseOffset(geometryBody) {
  if (!geometryBody) return { x: 0, y: 0 }
  const pointRe = /<mxPoint\b([^>]*?)\/>/gi
  let match
  let offsetAttrs = null
  while ((match = pointRe.exec(geometryBody)) !== null) {
    if (attr(match[1], 'as') === 'offset') {
      offsetAttrs = match[1]
      break
    }
  }
  if (!offsetAttrs) return { x: 0, y: 0 }
  const x = Number(attr(offsetAttrs, 'x'))
  const y = Number(attr(offsetAttrs, 'y'))
  return {
