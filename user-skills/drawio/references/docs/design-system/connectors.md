...
**Purpose**: Dependencies, constraints, composition in non-UML themes
...
For `meta.theme: uml-classic`, use the UML `dependency` relationship in the UML Relationship Types section below.
...
Use these connector types with `meta.theme: uml-classic` for classic UML diagrams.

| Type | Line | Arrow | Direction Convention |
|------|------|-------|----------------------|
| `association` | Solid 1px | None | Either direction |
| `directedAssociation` | Solid 1px | Open arrow at target | Source navigates to target |
| `aggregation` | Solid 1px | Hollow diamond at source | Source is the whole |
| `composition` | Solid 1px | Filled diamond at source | Source owns the part |
| `generalization` | Solid 1px | Hollow triangle at target | Source specializes target |
| `realization` | Dashed 1px | Hollow triangle at target | Source implements target |
| `dependency` | Dashed 1px | Open arrow at target | Source depends on target |
